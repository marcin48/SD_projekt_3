...
//Funkcje stworzeone na potrzeby bada� i wizualizacji nie maja one bezpo�rednio wp�ywu na wyniki bada� 
...
int Bucket::getRandomKeyFromBucket() {
	if (bst == nullptr) {
		return -1;  // Brak element�w w Buckecie
	}
...
void Bucket::printTree(Node* node, int level) {
	if (node != nullptr) {
		...
		cout << setw(4 * level) << ""; // Wci�cie poziome
		cout << "(" << node->entry->key << ", " << node->entry->value << ")" << endl; // Wy�wietlenie pary klucz-warto��
